...
### Fixed
- Removed `usableAsTool: true` from `ClientifyTrigger`: trigger nodes cannot be invoked as AI tools. `ClientifyApi` keeps it.
- Translated the code comments in `ClientifyTrigger.node.ts` to English.
